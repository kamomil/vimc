...
static void vimc_deb_configfs_cb(struct config_group *parent,
				 struct config_group childs[])
{
	config_group_init_type_name(&childs[0],
			VIMC_CFS_SINK_PAD_NAME(0),
			&vimc_default_cfs_pad_type);
	configfs_add_default_group(&childs[0], parent);
	config_group_init_type_name(&childs[1],
			VIMC_CFS_SRC_PAD_NAME(1),
			&vimc_default_cfs_pad_type);
	configfs_add_default_group(&childs[1], parent);
}
